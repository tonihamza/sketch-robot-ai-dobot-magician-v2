...
class StudioTests(unittest.TestCase):
    def test_active_robot_thumbnail_survives_next_photo_and_ai_result(self):
        ...
        try:
            ...
            studio.generating();studio.update_drawing_progress(42.8)
            self.assertEqual(studio.active_drawing.tobytes(),frozen)
            self.assertIn('42%',studio.drawing_caption.get());self.assertIn('se generează portretul',studio.message.get())
